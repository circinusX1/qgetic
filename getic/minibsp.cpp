...
//------------------------------------------------------------------------------------------
static  int32_t			Gdepth          = 0; //compiler does for globals
static  int32_t			GMaxDepth       = 0;
static  int32_t			GLoops           = 0;
static  int32_t			GPolys          = 0;
...
{
    u_bool32 operator()( Poly& a,  Poly& b) const
    {
...
{
    u_bool32 operator()( const Poly& a, const Poly& b) const
    {
        return ((Poly&)a).GetAreea() > ((Poly&)b).GetAreea();
    }
};
...
struct S_SortBySplitIdx
{
    u_bool32 operator()(const Poly& a,const  Poly& b) const
    {
        return (u_int32_t)a._use2splitidx < (u_int32_t)b._use2splitidx;
    }
};


//------------------------------------------------------------------------------------------
u_bool32 MiniBsp::Compile(vvector<Poly>& polys, u_int32_t brFlags, u_bool32 bLog, u_bool32 btextConvex)
{
    Clear();
    PolyList		loco;
    u_bool32			buildDummy  = false;
    _treeflags        = brFlags;
    Gdepth        = 0;
    GLoops        = 0;
    _reachedDepth = 0;
    ...
    if(IsMainModel())
    {
        ...
        loco.sort(S_SortBySplitIdx());

        if(COMP()->_bPolySortCrit==1 )
        {
            secondpart.sort(S_SortByAreea());					   // sort by area
        }
        else if(COMP()->_bPolySortCrit==2)
        {
            secondpart.sort(S_SortByNormal());
        }
    ...
    v3d_t center = Root()->_bbox.GetCenter();
    ...
    catch(EX_BSP e)
    {
        ...
        if(ThrID == GetCurrentThreadId())
            TAfxMessageBox("Error Compiling BSP");
...
//------------------------------------------------------------------------------------------
void MiniBsp::R_TerrainCompile(int32_t nodeIdx, PolyList& polys)
{
    static      int32_t Deepth = 0;
    PolyList	    frontPolys;
    PolyList	    backPolys;
...
#endif
//------------------------------------------------------------------------------------------
u_bool32 MiniBsp::_FormConvex(PolyList& frontPolys)
{
...
void MiniBsp::R_Compile(int32_t nodeIdx, PolyList& polys)
{
    ...
    int32_t splitters = 	Partition(pNode, polys, frontPolys, backPolys);
...
//------------------------------------------------------------------------------------------
// returns
int32_t  MiniBsp::FindPLane(const Plane& p)
{
    vvector<Plane>::iterator fp = _planes.findelement(p);
    if(fp == _planes.end())
    {
        int32_t index = _planes.size();
        _planes << p;
        return index;
    }
...
// suitable splitter
int32_t MiniBsp::GetBestSplitter(PolyList& polys, Poly* pWantPoly)
{
    ...
    FOREACH(PolyList, polys, pPoly1)
    {
        ...
        if(!polySpliter.IsSplitter())//_polyflags & SPLITTER_POLY
        {
            ...
            score = abs( int32_t((fronts-backs)+(boths*_balance)) );
            if(score < bestscore)
            {
                bestscore = score;
                bpIt  = pPoly1;
            ...
            maxfronts = tmax((u_int32_t)maxfronts, (u_int32_t)fronts);
            maxbacks =  tmax((u_int32_t)maxbacks,  (u_int32_t)backs);
    ...
    {
        if(polys.size() > (u_int32_t)Compiler::PCompiler->_maxFacesLeaf)
        {
            if((u_int32_t)maxbacks >= polys.size()-1) // still convex
            {
                const Plane& plane = GetCSGTerOptimSpliterPlane(polys, 0);
    ...
    if(0 == splitCnt)
    {
        qDebug("something wrong %d. cannot det front back", GLoops);
        assert(0);
    }
#endif //__FORCE_BSP
...
//------------------------------------------------------------------------------------------
void MiniBsp::Trace(int32_t idx)
{
    ...
    if(pNode->_nodeflags & NODE_LEAF)
    {
        ...
        {
            qDebug("p:%d->n:%d[X]", pNode->_idxParent, pNode->_idxNodeThis);
        }
        else
        {
            qDebug("p:%d->n:%d / l:%d) [polys:%d]",
                    pNode->_idxParent,
                    pNode->_idxNodeThis,
                    pNode->_leafIdx,
                    pNode->_nPolys);
    ...
    {
        qDebug("{(%d)>(%d)}",idx,pNode->_nodeidx[1]);
    ...
    if(pNode->_nodeidx[0]>=0)
    {
        ...
        qDebug("{(%d)<(%d)}",idx,pNode->_nodeidx[0]);
...
void	MiniBsp::BuildPlaneArray(PolyList& polys, u_int32_t flags)
{
...
void    MiniBsp::ClipBrPolys(vvector<Poly>& polySoup,
                          ...
                          u_bool32 bRemCoplan,  // remove any coplanars poly
                          u_bool32 bRemSolid,   // removes the splits that end up in solid space
                          u_bool32 bremoveUs)   // removes any unnecesarely splits after operation
{
    int32_t idx = 0;
...
void MiniBsp::R_ClipPolyList(int32_t idxNode,
...
void collect(int32_t index, vvector<Poly*>& fragments, vvector<Poly>& outs)
{
...
int32_t    MiniBsp::AddUniqueVertexes(vvector<v3d_t>& globPool)
{
    int32_t vertexes = 0;
    FOREACH(vvector<Poly>, _polys, pPoly)
    {
        FOREACH(vvector<Vtx2>, pPoly->_vtci, pVtci)
        {
            ...
            vvector<v3d_t>::iterator fi = find(globPool.begin(), globPool.end(), v);
            if(fi == globPool.end())
            {
...
int32_t	 MiniBsp::Partition(CMiniNode* pNode, PolyList& polys, PolyList&  frontPolys, PolyList& backPolys)
{
    if(Compiler::PCompiler->_break)
        throw 0;
...
Plane   MiniBsp::GetCSGTerOptimSpliterPlane(PolyList& polys, int32_t moe)
{
    Box allBox;
    Box oneBox(V0,v3d_t(100, 100, 100));
    ...
    // dim in half
    //
    v3d_t allEx  = allBox.GetExtends();
    v3d_t oneEx  = oneBox.GetExtends();
    v3d_t ex     = allBox.GetExtends();
    v3d_t c      = allBox.GetCenter();

    f32_t    mx = tmax(ex.x , ex.y);
    mx         = tmax(mx,    ex.z);
    ...
    if(moe)
    {
        ...
        if((allEx.x <=  oneEx.x &&
           allEx.y <= oneEx.y   &&
           allEx.z <=  oneEx.z) || polys.size() <= (u_int32_t)moe)
        {
...
u_bool32 MiniBsp::BuildDummyBSP(vvector<Poly>& polys)
{
...
u_bool32    MiniBsp::_CheckSafeStack(int32_t backSize)
{
    if(0 ==_nSafeStack)
    {
    ...
    for(int32_t s=0; s < _nSafeStack; s++)
    {
...
//------------------------------------------------------------------------------------------
u_bool32 MiniBsp::AllSpliters(PolyList& polyList)
{
    int32_t splcount = 0;
