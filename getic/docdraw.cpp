...
//-----------------------------------------------------------------------------
static void D32LocalDrawWireBrush(View2D *pV, Brush* pbr, int32_t mode ,
                                  u_bool32 bPOints, u_bool32 pselState, u_int8_t *pclr)
{
    f32_t    h  = pbr->_box.GetExtends().y;
    f32_t    m =  pbr->_box._min.y;
    int32_t     index = 0;
    v3d_t      pcenter;
    u_int8_t    lc[4];
    ...
    if(pbr->_brushflags & BRSH_NEW && pV)
    {
        ...
        VIEW3D(0,0)->PrintOut(pbr->_box.GetCenter(), pV->_vt, "Hit Enter", color);
    }
    Ta.Restore();
}

//-----------------------------------------------------------------------------
static void LocalDrawSelPolys(Poly** pPS , int32_t nCount, View2D* pV)
{
    int32_t i;
    Ta.Disable();
    Poly** pPSCache =  pPS;

    for(int32_t i=0; i < nCount; i++, pPS++)
    {
...
//-----------------------------------------------------------------------------
void  Document::LocalDrawBspTreeGraph(Compiler& compiler, int32_t& leafNo, View3D* pV)
{
    v3d_t  vtree;


    int32_t nLeaf = compiler._bTrees[0]->R_FindLeaf(0,_cam._pos);
...
//-----------------------------------------------------------------------------
void Document::LocalDrawReadyBSP(Compiler&  compiler, Scene& scene, View3D* pV)
{
    if(_clearing)
        return;

    int32_t nLeaf = compiler._bTrees[0]->R_FindLeaf(0,_cam._pos);
    u_int32_t zoneVis = 0xFFFFFFFF;
...
//-----------------------------------------------------------------------------
u_bool32 Document::InitGL(int32_t i3d, HDC hDC, HGLRC hrc)
{
/*?*
...
//-----------------------------------------------------------------------------
void  Document::SetLightMode(u_bool32 invalidate)
{
    //MakeCurrent('3');
    if(!GLight)
    {
...
//-----------------------------------------------------------------------------
static u_int8_t* S_SelectColor(Brush& rb,u_bool32 sel=false)
{
    if(sel)
    {
...
//-----------------------------------------------------------------------------
void Document::Draw2DBrushes(View2D *pV)
{
    if(_clearing)
        return;
    ...
    glColor4ubv(CLR_WHITE);
    for(int32_t i=0; i < nCount; i++, pBS++)
    {
        //
    ...
    if(_selEdges.size())
    {
        ...
        {
            glVertex3fv((f32_t*)edge->_edge[0]->_xyz);
            glVertex3fv((f32_t*)edge->_edge[1]->_xyz);
        }
        glEnd();
        glLineWidth (1.0f);
...
//-----------------------------------------------------------------------------
void    Document::Draw3DScene(View3D* pV)
{
    if(_clearing)
        return;
    ++PaintCounter;
    ...
    do{
        ...
        if(_compviewmode != C_NA )
        {
            ...
            {
                f32_t frgb[4];
                frgb[0]=(f32_t)_scene._si.fogColor.r/255.0;
                frgb[1]=(f32_t)_scene._si.fogColor.g/255.0;
                frgb[2]=(f32_t)_scene._si.fogColor.b/255.0;
                frgb[3]=1.0;
                fe = 1;
...
//-----------------------------------------------------------------------------
void    Document::DrawDebugPointsAndPolys()
{
    Ta.Disable();
    ...
    if(_testPoints.size())
    {
        FOREACH(vvector<v3d_t>, _testPoints, pPt)
        {
...
//---------------------------------------------------------------------------------------
void  Document::Draw3DRawScene(View3D* pV)
{
    if(_clearing)
        return;
    _maxMirrorsDraws=0;
...
//---------------------------------------------------------------------------------------
void    Document::DrawCompiledStates(View3D* pV)
{
    _compiler.Render(pV,0xFF);
}

//---------------------------------------------------------------------------------------
void Document::DrawTextured(View3D *pV)
{
    if(_clearing || _maxMirrorsDraws > 2)   return;

    ++_maxMirrorsDraws;
    vvector<Brush*> lastones(256);
    u_bool32            selOn =   (SEL_BRUSH  == _selmode || SEL_FACE   == _selmode || SEL_VERTEX == _selmode);
...
void Document::DrawItems()
{
    ...
    int32_t cnt;
    SceItem** it = Pd->GetSelItems(cnt);
    for(int32_t i=0;i<cnt;i++,it++)
    {
...
//---------------------------------------------------------------------------------------
void Document::DrawProjectorBrush(Brush& rb)
{
    ...
    u_int8_t* pcrl = S_SelectColor(rb, rb.IsSelected());
...
//---------------------------------------------------------------------------------------
void Document::DrawCutBrush(Brush& rb)
{
    Ta.Disable();
    if(BShowCb)
    {
    ...
    u_int8_t* pclr=S_SelectColor(rb, rb.IsSelected());
...
//---------------------------------------------------------------------------------------
void Document::DrawSolidBrush(Brush& rb,
                               vvector<Poly*>& backDraw,
                               vvector<Poly*>& transPolys,
                               vvector<Poly*>& mirrors,
                               vvector<Poly*>& masked,
                               ...
                               u_bool32 usesel)
{
    ...
    for(;pb!=pe;pb++)
    {
        ...
        if(usesel)
        {
            ...
            glColor4ubv((u_int8_t*)pl._colorD);
...
//---------------------------------------------------------------------------------------
void    Document::ApplyCloserLights(const Poly& poly)
{
    CLightBulb* pL;
    f32_t position[4];
    f32_t dl[4];
    f32_t sl[4];
    f32_t e[4] = {poly._colorE.r/255.0f, poly._colorE.g/255.0f, poly._colorE.b/255.0f, 1.0f};
    f32_t d[4] = {poly._colorD.r/255.0f, poly._colorD.g/255.0f, poly._colorD.b/255.0f, 1.0f};
    f32_t s[4] = {poly._colorS.r/255.0f, poly._colorS.g/255.0f, poly._colorS.b/255.0f, 1.0f};
    ...
    int32_t nLightCount = 0;
    int32_t maxLights = _maxLights;

    for(int32_t i=0; i < _maxLights; i++)
...
static void __SetFogColor(v3d_t& v, u_rgba_t& color)
{
    float z;	/* distance of vertex from edge of fog volume */
    float f;	/* fog factor */
    v3d_t&  cp = Pd->_cam._pos;
    z = 0.0 - v.y;
...
//---------------------------------------------------------------------------------------
void Document::DrawSpecialPolygons(View3D *pV,
                                    u_int32_t lmaps,
                                    vvector<Poly*>& backDraw,
                                    vvector<Poly*>& transPolys,
                                    vvector<Poly*>& mirrors,
                                    ...
                                    vvector<Poly*>& guround)
{
    ...
    if(mirrors.size())
    {
        ...
        {
            pPoly = *ppPoly;
            DrawPolyVertexes(GL_POLYGON, *pPoly, 0, (u_int32_t)-1);
        }
...
void Document::DrawSelBrushes(View3D *pV)
{
    int32_t     nCount;
    Brush** pBS         = Pd->GetSelBrushes(nCount);
...
//---------------------------------------------------------------------------------------
void Document::DrawSelFaces(View3D *pV)
{
    int32_t     polys,i;
    Poly**  pPI = Pd->GetSelPolys(polys);
    Poly**  pPS = pPI;
...
//---------------------------------------------------------------------------------------
void Document::DrawAxis(v3d_t& org, f32_t ex)
{
   Ta.Disable();
    glPushMatrix();
...
//---------------------------------------------------------------------------------------
void Document::ReposCamLight(u_int32_t lightbits, int32_t avaliCnt, f32_t offset, u_rgba_t difLight)
{
    ...
    f32_t a[4]={ _scene._si.defLumin.r/255.0,
                _scene._si.defLumin.g/255.0,
                _scene._si.defLumin.b/255.0, 1.0};

    f32_t d[4]={difLight.r, difLight.g, difLight.b, 1.0};
    f32_t s[4]={1.0, 1.0, 1.0, 1.0};
    f32_t linearAttenuation       = .00001;
    float cut_off                = 360.0;
...
//---------------------------------------------------------------------------------------
void Document::DrawMotionsPaths()
{
    Ta.Disable();
...
//---------------------------------------------------------------------------------------
void Document::Draw2DScene(View2D *pV)
{
    if(_clearing) return;
...
//---------------------------------------------------------------------------------------
void Document::Draw2dItems(View2D *pV)
{
    TexRef::GlDecalMode(1);
    FOREACH(vvector<SceItem*>, (*GetItems()), ppi)
    {
...
//-----------------------------------------------------------------------------
void Document::DrawCameraRay(View2D *pV)
{
    glEnable(GL_BLEND);
...
//-----------------------------------------------------------------------------
void Document::DrawCamFrustrum(View2D *pV)
{
    int32_t i;
    if(pV->_zoom > 1.f)
    {
...
//-----------------------------------------------------------------------------
void    Document::GLDrawBox(Box& box, u_bool32 asWire, RGBA color)
{
    Ta.Disable();
    glColor4ubv(color);
...
//-----------------------------------------------------------------------------
u_int32_t Document::SetFaceMode(u_bool32 resetAll)
{
    if(resetAll)
    {
        Ta.Reset();
...
void Document::DrawPolygon(Poly& poly, u_bool32 wireMode)
{
 //TODO???  if(poly._n.x>=0.999999f)
 //       qDebug("............");

    //qDebug("TODO");


    if(_vmodefire)// && theApp->get_modifiers(Qt::Key_Control))
    {
        Ta.Disable();
...
static v3d_t Dir2Euler(const v3d_t& dir, const v3d_t& up)
{
    v3d_t euler;
    f32_t dy2 = (f32_t)sqrt(1.0-dir.y*dir.y);
    euler.x = (f32_t)Sinr(dir.y);
    euler.z = (f32_t)Cosr(up.y  / dy2);
    euler.y = (f32_t)Sinr(dir.x / dy2);	  // az
    ROLLPI(euler.y);
    return euler;
}
...
//MERGE//>
void    Document::DrawOrtho(const v3d_t& pos, const v3d_t& euler, f32_t len, f32_t clratn)
{
    int32_t i;
    Ta.Disable();
    glPushMatrix();
    ...
    f32_t    ld      = len/6.0;
    int32_t     nVerts  ;
    v3d_t*     pV3      = __GetCone(v3d_t(0,0,len), lh, ld, nVerts);

        glPushMatrix();//Z
            glTranslatef(0, 0, len);
            glBegin(GL_TRIANGLE_FAN);
                glColor4ub(0,0,255*clratn,255);
                glVertex3fv((f32_t*)pV3[0]);
                glColor4ub(0,0,192*clratn,255);
                for(int32_t i=1;i<nVerts;i++)
                {
                    glVertex3fv((f32_t*)pV3[i]);
                }
            glEnd();
        glPopMatrix();
...
static void AnimateSurfaceTexture(const Poly& face, v2d_t& tc, int32_t ts)
{

    u_int32_t texBits = (face._polyflags2 & 0xF);
    if(texBits==0) texBits=1; // fix if tanim flag is set animate first stage
    if(texBits & (1 << ts))
    {
        if(face._polyprops & FACE_TA_RAD)
        {
            static int32_t locoCnt = 1;
            v2d_t ext((f32_t)locoCnt*face._tanimA.u, (f32_t)locoCnt*face._tanimA.v);
            if(tc.u < .5)
            {
                tc.u += ext.u;
                if(tc.u>.5)
        ...
        {
            v2d_t tcf = v2d_t(1/face._tanimA.u, 1/face._tanimA.v);
            tc     *=  tcf;
            tcf    *= (f32_t)PaintCounter;
            tcf.u-= ceil(tcf.u);
            tcf.v-= ceil(tcf.v);
        ...
        {
            v2d_t depl(face._tanimA.u * Sinr(PaintCounter/16.0), face._tanimA.v * Sinr(PaintCounter/16.0));
            tc+=depl;
            return;
        }
        ...
        {
            v2d_t depl(face._tanimA.u * Sinr(PaintCounter/16.0), face._tanimA.v * Cosr(PaintCounter/16.0));
            tc+=depl;
            return;
        }
        ...
        {
            v2d_t depl(face._tanimA.u * PaintCounter, face._tanimA.v * PaintCounter);
            tc+=depl;
             return;
        }

        //v2d_t depl(FRAND(100,1000), FRAND(100,1000));
        //tc+=depl;
        return;
...
void Document::DrawPolyVertexes(u_int32_t how, const Poly& poly, int32_t texCoord, u_int32_t what)
{
    u_int8_t    farclr[4] = {255,255,255,255};
    v2d_t      texC;
    u_bool32    clrchangec = 0;
    //glColor4ubv(CLR_WHITE);
    glBegin(how);
        CPVertex vb = poly._vtci.begin();
        CPVertex ve = poly._vtci.end();
        for(;vb!=ve;vb++)
        {
            if(_compviewmode & C_BYFAR)
            {
                farclr[0] = u_int8_t((vb->_xyz.x/Poly::__Max.x)*255);
                farclr[1] = u_int8_t((vb->_xyz.y/Poly::__Max.y)*255);
                farclr[2] = u_int8_t((vb->_xyz.z/Poly::__Max.z)*255);
                glColor4ubv((u_int8_t*)farclr);
            }
...
void Document::DrawPolyLines(Poly& poly)
{
    PVertex vb,ve;
    glBegin(GL_LINE_LOOP);
        vb = poly._vtci.begin();
...
void Bezier::tessellate(int32_t L) {
    level = L;

    // The number of vertices along a side is 1 + num edges
    const int32_t L1 = L + 1;
...
void   Document::DrawRect(Htex& texture, const Box& box)
{
    Ta.Enable();
    Ta.Bind(texture);
